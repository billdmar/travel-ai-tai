...
function MapLink({ activity }: { activity: Activity }) {
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-medium text-slate-500 transition hover:text-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
    >
      <svg
        aria-hidden="true"
...
function BookLink({ activity }: { activity: Activity }) {
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-md border border-brand-200 px-2 py-1 text-xs font-semibold text-brand-700 transition hover:border-brand-300 hover:bg-brand-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
    >
      <svg
        aria-hidden="true"
        className="h-3.5 w-3.5"
...
export default function DayCard({ day, grandTotal, defaultOpen = false }: DayCardProps) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition hover:bg-slate-50/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-500"
      >
        <div className="flex items-center gap-4">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-50 text-sm font-semibold text-brand-700 ring-1 ring-brand-100">
            {day.day_number}
          </span>
          <div>
      ...
      {grandTotal != null && (
        <div aria-hidden="true" className="h-px bg-slate-100">
          <div className="h-px bg-brand-400" style={{ width: `${sharePct}%` }} />
        </div>
      )}
            ...
            {day.activities.map((a, i) => (
              <li key={i} className="rounded-xl border border-slate-100 p-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-slate-600">{a.time}</span>
                  <span className="text-sm font-semibold tabular-nums text-slate-900">
                    {money(a.estimated_cost_usd)}
                  </span>
                </div>
                <p className="mt-1 font-medium text-slate-900">{a.place}</p>
                <p className="text-xs leading-relaxed text-slate-500">{a.description}</p>
            ...
            <table className="w-full text-left text-sm">
                ...
                {day.activities.map((a, i) => (
                  <tr key={i} className="border-t border-slate-100 align-top">
                    ...
                    <td className="px-3 py-3.5">
                      <p className="font-medium text-slate-900">{a.place}</p>
                      <p className="text-xs leading-relaxed text-slate-500">{a.description}</p>
                    </td>
                    <td className="px-3 py-3.5">
                      <CategoryChip category={a.category} />
                    </td>
                    <td className="whitespace-nowrap px-3 py-3.5 text-right text-sm tabular-nums text-slate-700">
                      {money(a.estimated_cost_usd)}
                    </td>
              ...
              <tfoot>
                <tr className="border-t border-slate-200 bg-slate-50/60">
                  <td
                    colSpan={3}
                    className="px-3 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-400"
                  >
                    Day subtotal
                  </td>
