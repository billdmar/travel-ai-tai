...
// only by a small accent dot. Keeps category cueing without the loud rainbow,
// in service of the minimal/elegant aesthetic and visual unity.
const categoryDot: Record<ActivityCategory, string> = {
  food: 'bg-accent-500',
  attraction: 'bg-accent-500',
  transport: 'bg-accent-500',
  accommodation: 'bg-accent-500',
  leisure: 'bg-accent-500',
  other: 'bg-accent-500',
}

function CategoryChip({ category }: { category: ActivityCategory }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-accent-100 px-2.5 py-0.5 text-xs font-medium capitalize text-accent-700">
...
function MapLink({ activity }: { activity: Activity }) {
  if (!activity.map_url) {
    return <span className="text-xs text-ink-line">—</span>
  }
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-medium text-ink-faint transition hover:text-accent-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
    >
      <svg
        aria-hidden="true"
...
function BookLink({ activity }: { activity: Activity }) {
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 rounded-md border border-accent-200 px-2 py-1 text-xs font-semibold text-accent-700 transition hover:border-accent-300 hover:bg-accent-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
    >
      <svg
        aria-hidden="true"
        className="h-3.5 w-3.5"
...
export default function DayCard({ day, grandTotal, defaultOpen = false }: DayCardProps) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-ink-line/80 bg-canvas-raised shadow-sm transition hover:border-ink-line hover:shadow-md">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition hover:bg-canvas-sunken/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500"
      >
        <div className="flex items-center gap-4">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-50 text-sm font-semibold text-accent-700 ring-1 ring-accent-100">
            {day.day_number}
          </span>
          <div>
            ...
            <h3 className="font-semibold text-ink">{day.theme}</h3>
        ...
        <div className="flex items-center gap-4">
          ...
          <svg
            aria-hidden="true"
            className={`h-5 w-5 text-ink-line transition-transform ${open ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <ul className="space-y-3 sm:hidden">
            {day.activities.map((a, i) => (
              <li key={i} className="rounded-xl border border-ink-line p-3">
                ...
                <p className="text-xs leading-relaxed text-ink-faint">{a.description}</p>
